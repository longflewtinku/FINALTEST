...
public class HistoryReceipt extends Receipt {
    ...
    public PrintReceipt generateReceipt(Object obj) {
        ...
        this.addOptoMerchantDetails(receipt, 1);

        Receipt.addLineCentered(receipt, getText(STR_HISTORY_REPORT).toUpperCase(), LARGE_FONT);

        Receipt.addLineCentered(receipt, getText(STR_SN) + mal.getHardware().getSerialNumber(), SMALL_FONT);
        if (UserManager.getActiveUser() != null && UserManager.getActiveUser().getTerminalId() != null && !UserManager.getActiveUser().getTerminalId().isEmpty())
            Receipt.addLineCentered(receipt, getText(STR_TID) + UserManager.getActiveUser().getTerminalId(), SMALL_FONT);
        ...
        if (rec.getRecTransList() == null || rec.getRecTransList().isEmpty()) {
            ...
            for (; t >= 0; t--) {
                ...
                StringBuilder transName = new StringBuilder(tran.getTransType().getDisplayName());
                while (transName.length() < 8) {
                    transName.append(" ");
                }
                transName = new StringBuilder(transName.substring(0, 7));
                ...
                receipt.getLines().add(new PrintReceipt.PrintTableLine(getText(STR_PAN) + tran.getMaskedPan(TransRec.MaskType.REPORT_MASK, d.getPayCfg()), getText(STR_PID)+ tran.getEftPaymentIdShort(d.getPayCfg().getStid())));
