...
public class ReportReceipt extends GenericBaseReceipt {
    public final IMessages.ReportType reportType;

    public ReportReceipt(IMessages.ReportType reportType) {
    ...
    public PrintReceipt generateReceipt(Object obj, TransRec trans) {
        ...
        long saleAmount = rec.getSale().amount - rec.getSale().reversalAmount;
         ...
         long cashbackCount = rec.getCashback().count - rec.getCashback().reversalCount;
         long cashbackAmount = rec.getCashback().amount - rec.getCashback().reversalAmount;
        ...
        this.addCountAndAmount(receipt, STR_CASH, cashCount + cashbackCount, cashAmount + cashbackAmount); // purch+cash transaction should be bundle with cashout transaction with net total of 1 transaction
        this.addCountAndAmount(receipt, STR_REFUND, refundCount, refundAmount, refundAmount > 0);
        ...
        this.addCountAndAmount(receipt, STR_NET_TOTAL, saleCount + cashCount + refundCount + preAuthCompleteCount, (saleAmount + cashAmount + cashbackAmount + preAuthCompleteAmount - refundAmount)); // We need to count purch+cash as 1 transaction so not adding to count but adding to amount
        this.addSpaceLines(receipt, 2);
