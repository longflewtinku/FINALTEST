...
public class Iso8583 extends AMessage implements IMessage {
    ...
    static {
        ...
        try {
            ...
            _defaultTemplate.put(Bit._015_SETTLEMENT_DATE, FieldDescriptor.getAsciiFixed(4, FieldValidators.getN()));
    ...
    public MSG_TYPE_ENCODING getMsgTypeEncoding() {
        return this.msgTypeEncoding;
    }

    public void setMsgTypeEncoding(MSG_TYPE_ENCODING msgTypeEncoding) {
        this.msgTypeEncoding = msgTypeEncoding;
    }

    public enum MSG_TYPE_ENCODING {
        BCD,
        ASCII
    }
    protected MSG_TYPE_ENCODING msgTypeEncoding = MSG_TYPE_ENCODING.BCD; // size in bytes (use 2 bytes for bcd encoding, or 4 if ascii encoded)
    ...
    public int getOffsetOfField(int fieldId) throws Exception {
        int superOffset = super.getOffsetOfField(fieldId);
        if (superOffset < 0) {
            return superOffset;
        }
        int mtiLen = template.getMsgTypeFormatter().getPackedLength(4);
        return superOffset + mtiLen;
    }
    ...
    public void setTransactionAmount(long transactionAmount) throws UnknownFieldException {
        ...
        setFieldValue(Bit._004_TRAN_AMOUNT, IsoUtils.padLeft(strAmt, 12, '0'));
    }
    ...
    public int unpack(byte[] msg, int startingOffset, boolean validateData) throws Exception {
        ...
        if( msgTypeEncoding == MSG_TYPE_ENCODING.BCD ) {
            byte[] buffer = new byte[2];
            System.arraycopy(msg, offset, buffer, 0, buffer.length);
            String msgTypeString = Util.bcd2Str(buffer);
            _msgType = (int) Long.parseLong(msgTypeString, 16);
        ...
        return super.unpack(msg, offset, validateData);
    }
    ...
    public static class MsgType {
        ...
        public static int toInt(String msgType) {
            return IsoUtils.msgTypeToInt(msgType);
        }

        public static String toString(int msgType) {
            return IsoUtils.msgTypeToString(msgType);
        }
    }
    ...
    public class Bit {
        ...
        public static final int _015_SETTLEMENT_DATE = 15;
