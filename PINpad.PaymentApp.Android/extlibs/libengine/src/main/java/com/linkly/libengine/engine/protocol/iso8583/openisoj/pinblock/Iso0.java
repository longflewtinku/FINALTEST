package com.linkly.libengine.engine.protocol.iso8583.openisoj.pinblock;
...
public class Iso0 {
     ...
     *
     * @param accountNumber (PAN) consists of the BIN (Bank Identification Number), accountNumber and a check digit.
    ...
    public byte[] createPinBlock(String pan, String pin) throws Exception {
        ...
        String pinData = IsoUtils.padLeft("" + pin.length(), 2, '0') + pin;
        pinData = IsoUtils.padRight(pinData, 16, 'F');
        byte[] bPin = Formatters.getBinary().getBytes(pinData);
        String acctData = IsoUtils.padLeft(acct, 16, '0');
