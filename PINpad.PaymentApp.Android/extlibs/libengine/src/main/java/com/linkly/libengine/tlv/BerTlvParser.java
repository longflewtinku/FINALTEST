...
public class BerTlvParser {
    ...
    public BerTlvs parseBuffer(byte[] aBuf, final int aOffset) {
        List<BerTlv> tlvs = new ArrayList<>();
        int bufferLen = aBuf.length;

        int offset = aOffset;
        while(true) {
            ParseResult result =  parseWithResult(0, aBuf, offset, bufferLen-offset);
            tlvs.add(result.tlv);

            if(result.offset >= bufferLen) {
                // end of buffer reached or exceeded
                break;
            }
    ...
    private ParseResult parseWithResult(int aLevel, byte[] aBuf, int aOffset, int aLen) {
        ...
        if(tag.isConstructed()) {
            ...
            addChildren(aLevel, aBuf, aOffset+tagBytesCount+lengthBytesCount, levelPadding, valueLength, list);
            if(log.isDebugEnabled()) {
     ...
     */
    private void addChildren(int aLevel, byte[] aBuf, int aOffset, String levelPadding, int valueLength, ArrayList<BerTlv> list) {
        ...
        while (startPosition < aOffset + valueLength) {
            ...
            len           = valueLength - startPosition + aOffset;
