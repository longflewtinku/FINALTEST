...
public abstract class AMessage implements Cloneable {
    ...
    /**
     * returns length (bytes) of length indicator
     *
     * @param fieldId index/id of field
     * @return -1 = unknown field, zero = fixed length field, > 0 = length of length (bytes)
     */
    public int getLengthOfLength(Integer fieldId) {
        IFieldDescriptor descriptor = template.get(fieldId);
        if (descriptor == null) {
            return -1;
        }
        return descriptor.getLengthFormatter().getLengthOfLengthIndicator();
    }

    /**
     * returns offset in current packed message of start of field
     * if variable length field, returns index of first length indicator byte
     *
     * @param fieldId index/id of field
     * @return -1 = field not found, >= 0, offset of field
     * @throws UnknownFieldException
     */
    public int getOffsetOfField(int fieldId) throws Exception {
        if (!bitmap.isFieldSet(fieldId)) {
    ...
    public int getFieldPackedLength(int fieldIdx) throws Exception {
        IField field = getField(fieldIdx);
    ...
    public void set(int field, int value) throws UnknownFieldException {
        setFieldValue(field, String.valueOf(value));
    }
    ...
    public void set(int field, long value) throws UnknownFieldException {
        setFieldValue(field, String.valueOf(value));
    }
    ...
    protected int toMsg(byte[] data, int offset, int fld) throws Exception {
        if (bitmap.isFieldSet(fld)) {
            ...
            if (field != null) {
                ...
                Timber.e("ERROR packing field %d, field data not found", fld);
            }
        }
        return 0;
    }
    ...
    protected String toString(String prefix) throws Exception {
        ...
        if( !Util.isNullOrEmpty( lineSeparator ) && sb.length() > lineSeparator.length() ) {
            return sb.substring( lineSeparator.length() );
        } else {
            return  null;
        }
    }

    protected int unpack(byte[] msg, int startingOffset, boolean validateData) throws Exception {
        ...
        for (int i = 2; i <= 128; i++) {
            if (bitmap.isFieldSet(i)) {
                ...
                offset = field.unpack(msg, offset, validateData);
                field.setOffset(offsetStart-startingOffset);
                field.setFieldLength(offset-offsetStart);
            }
        }
