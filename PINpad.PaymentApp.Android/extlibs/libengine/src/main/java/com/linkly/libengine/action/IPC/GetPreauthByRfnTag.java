...
public class GetPreauthByRfnTag extends IAction {
    @Override
    public String getName() {
        return "GetPreauthByRfnTag";
    }
    ...
    public void run() {
        ...
        if (Util.isNullOrEmpty(rfn)) {
            // RFN tag not present, cancel the transaction
            ...
            if (!lookupCardDetailsFromRfnTag(rfn)) {
                ...
                // set cardholder not present by default for all integrated (lookup by RFN tag) transactions
    ...
    private boolean lookupCardDetailsFromRfnTag(String rfn) {
        ...
        TransRec originalTxn = d.getProtocol().lookupOriginalTransaction( rfn );
        if( originalTxn == null ) {
